    ...
    function (
      ...
      $scope.loginAsGuest = function () {
        ...
        if ($rootScope.randomToken) {
          checkGuestLoginStatus($rootScope.randomToken);
        }
      };

      function checkGuestLoginStatus(token) {
        Restangular.one("user")
          .post("guest_access", JSON.stringify({ token: token }), undefined, {
          ...
          .then(
            function (resp) {
              ...
              if (
                status === 2 &&
                resp.username &&
                (resp.password || localStorage.password)
              ) {
