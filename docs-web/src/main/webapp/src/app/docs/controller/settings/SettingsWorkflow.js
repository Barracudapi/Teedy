...
/**
 * Settings workflow and activity controller.
 */
angular
  .module("docs")
  .controller(
    "SettingsWorkflow",
    function ($scope, $state, Restangular, $translate) {
      // Initialize variables for both workflows and activities
      ...
      $scope.editWorkflow = function (workflow) {
        $state.go("settings.workflow.edit", { id: workflow.id });
      };
      ...
      function prepareGanttData() {
        ...
        $scope.activities.forEach(function (activity) {
          ...
          var startDate = activity.create_timestamp
            ? new Date(activity.create_timestamp)
            : new Date();
          var endDate;
          ...
          userGroups[activity.username].push({
            id: activity.id,
            name: activity.entity_name || "Activity",
            start: startDate,
            end: endDate,
        ...
        Object.keys(userGroups).forEach(function (username) {
          ...
          tasks.forEach(function (task) {
            ...
            var overlappingTasks = processedTasks.filter(
            ...
            if (overlappingTasks.length > 0) {
              ...
              var usedPositions = overlappingTasks.map(function (t) {
      ...
      $scope.deleteActivity = function (activity) {
        if (
          confirm($translate.instant("settings.user_activities.confirm_delete"))
        ) {
          Restangular.one("useractivity", activity.id)
            .remove()
            .then(function () {
